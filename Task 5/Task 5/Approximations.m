% --- Longitudinal Models ---
    ...
    A_FL = [Xu, Xw, -w0, -g*cos(theta0) ;
        ...
        Mu+Mw*(Zu/(1-Zwdot)), Mw+Mw*(Zw/(1-Zwdot)), Mq+Mw*(Zq+u0)/(1-Zwdot), -Mw*(-g*sin(theta0))/(1-Zwdot);
        0, 0, 1, 0];
    B_FL = [Xde, Xdth;
    ...
    %TFs
    Full_long_tf = tf(sys_FL_long);
    %Elevator
    u_de = Full_long_tf(1,1);
    w_de = Full_long_tf(2,1);
    q_de = Full_long_tf(3,1);
    theta_de = Full_long_tf(4,1);
    wdot_de = w_de*Differentiator;
    az_de = wdot_de-u0*q_de;
    ...
    %Thrust
    u_dT= Full_long_tf(1,2);
    w_dT= Full_long_tf(2,2);
    alpha_dT= w_dT/u0;
    q_dT= Full_long_tf(3,2);
    theta_dT= Full_long_tf(4,2);
    
    %Pitch control theta/theta command
    %Open loop TFs
    OL_theta_thetacmd = -servo*theta_de;
    OL_u_ucmd = servo*engine_timelag*u_dT;
    % h_theta = minreal(-Integrator*(w_de/theta_de-u0));
    % OL_h_thetacmd = minreal(CL_theta_thetacmd*h_theta);

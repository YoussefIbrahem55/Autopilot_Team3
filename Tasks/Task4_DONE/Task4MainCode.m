...
for k = 1:N
   ...
   Approximations;
    ...
    conversion1=[1,1,180/pi,180/pi];
    fullLong=fullLong.*conversion1;
    longPeriod=longPeriod.*[1,180/pi];
    shortPeriod=shortPeriod.*[1,180/pi];
    %% ====================== RESULT PLOTTING ======================
    if long_or_lat == 1
...
for i = 1:numTables
    ...
    sys_mod = sys_FL_long;%modred(sys_FL_long, modRedIndices{i}, 'truncate');
        
        systemsTable.sys = {sys_mod; approxModels{i}};
    ...
    export_system_comparison(systemsTable, stateNames, inputNames,outputIndices{i});
end
...
for i = 1:numTables
    ...
    % Modify system and select outputs
    sys_mod = sys_FL_lat;   %modred(sys_FL_lat, modRedIndices{i}, 'truncate');
    if i==numTables
        systemsTable.sys = {sys_mod(:,1); approxModels{i}};
    else
    ...
    % Store table and run comparison
    systemsTables{i} = systemsTable;
    export_system_comparison(systemsTable, stateNames, inputNames,outputIndices{1});
end
end

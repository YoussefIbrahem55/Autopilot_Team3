function export_system_comparison(systemsTable, stateNames, inputNames,index)
    ...
    for i = 1:size(sys2.B, 2)  % Loop over inputs
        
        for j = 1:size(sys2.C, 1)  % Loop over states
            % Get transfer functions for both systems
            G1_ij = minreal(tf1(j, i));
            G2_ij = minreal(tf2(j, i));
...
%     sgtitle(sprintf('%s vs %s', systemsTable.modeName{1}, systemsTable.modeName{2}), 'FontWeight', 'bold');
     % Get frequency response data
    bode(G1, 'b', G2, 'r--');  % Blue for full model, red dashed for approximation
    legend(systemsTable.modeName{1},systemsTable.modeName{2}, 'Location', 'best');
    title(sprintf('Bode Magnitude: $$\\frac{%s}{%s}$$', stateName, inputName), 'Interpreter', 'latex');
   % Save plot
